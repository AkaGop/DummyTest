# log_parser.py
import re; from io import StringIO; from config import CEID_MAP, RPTID_MAP; from parser_utils import tokenize, build_tree
def _parse_s6f11(text):
    data = {}; tree = build_tree(tokenize(text))
    try:
        ceid, rptid, payload = int(tree[0][1]), int(tree[0][2][0][0]), tree[0][2][0][1]
        if ceid in CEID_MAP:
            data['CEID'] = ceid
    ...
    return data
def _parse_s2f49(text):
    data = {}; rcmd = re.search(r"<\s*A\s*\[\d+\]\s*'([A-Z_]{5,})'", text)
    if rcmd: data['RCMD'] = rcmd.group(1)
    lotid = re.search(r"'LOTID'\s*>\s*<A\[\d+\]\s*'([^']*)'", text, re.IGNORECASE)
    if lotid: data['LotID'] = lotid.group(1)
    panels = re.search(r"'LOTPANELS'\s*>\s*<L\s\[(\d+)\]", text, re.IGNORECASE)
...
def parse_log_file(file):
    ...
    i = 0
    while i < len(lines):
        line = lines[i].strip(); header = re.match(r"(\d{4}/\d{2}/\d{2} \d{2}:\d{2}:\d{2}\.\d+),\[([^\]]+)\],(.*)", line)
        if not header: i += 1; continue
        ts, ltype, msg_part = header.groups(); msg_match = re.search(r"MessageName=(\w+)|Message=.*?:\'(\w+)\'", msg_part)
